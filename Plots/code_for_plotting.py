...
for i in range(len(lis)):
    temp = df_2.loc[df_2['delay'] == lis[i]]
    plt.plot([float(i) for i in temp['corrupt'].to_list()],[float(i) for i in temp['throughput'].to_list()])
    plt.legend(lis[:i+1], loc='upper left')
...
plt.plot([float(i) for i in df_2['delay'].to_list()],[float(i) for i in df_2['throughput'].to_list()])
...
plt.plot([float(i) for i in df_2['reorder'].to_list()],[float(i) for i in df_2['throughput'].to_list()])
